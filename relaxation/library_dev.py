...
# library manager
# needs to track global vs. local variables
# block solve functions return library things bookshelves??)
# and accept library objects as input
...
def block1_solve(v_init: list,
    ...
    def jax_obj(v):
        ...
        c_l = consensus([l1, l_init_2]) # consensus for l
        c_mp = consensus([mp1, mp_init_2]) # consensus for mp
...
def block2_solve(v_init: list,
    ...
    def jax_obj(v):
        ...
        c_l = consensus([l_init_1, l2]) # consensus for l
        c_mp = consensus([mp_init_1, mp2]) # consensus for mp
        c = jnp.concatenate((c_l, c_mp)) # consensus for all global vars
